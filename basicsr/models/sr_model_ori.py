...
class SRModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        self.optimizer_g.zero_grad()
        self.output = self.net_g(self.lq)  # 这里返回bic lr和sr,然后增加loss
        l_total = 0
        loss_dict = OrderedDict()
        # pixel loss
        if self.cri_pix:
        ...
        with torch.no_grad():  # 在这里增加测试模型融合
            for rot in range(0, 4):
                for flip in [False, True]:
                    _x = x.flip([1]) if flip else x
                    _x = _x.rot90(rot)
                    out_sr = self.net_g(_x.permute(2, 0, 1).unsqueeze(0).clone())

                    out_sr = out_sr.squeeze(0).permute(1, 2, 0)
                    out_sr = out_sr.rot90(4 - rot)
                    out_sr = out_sr.flip([1]) if flip else out_sr  # 720, 1280, 3

                    # out_lr = out_lr.squeeze(0).permute(1, 2, 0)
                    # out_lr = out_lr.rot90(4 - rot)
                    # out_lr = out_lr.flip([1]) if flip else out_lr  # 720, 1280, 3
                    # out_sr = self.back_projection(3, out_sr, x)
                    sr.append(out_sr)
